...
const transferSPL = async () => {
  ...
  ); // Phantom account for testing, it already has a associated account
...
const lookupRedeemSPL = async (mintAddress: string) => {
  ...
  if (!destAccountInfo)
    inst.push(
      await Token.createAssociatedTokenAccountInstruction(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, mintAccount, dest, receiver, signer)
    );
